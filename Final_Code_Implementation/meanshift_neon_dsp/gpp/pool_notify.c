...
{
    NOTIFY_notify (0, pool_notify_IPS_ID, pool_notify_IPS_EVENTNO, notif_payload);
}
...
Void write_buffer(uint32_t size, Uint32 notif_payload)
{
    ...
    printf("[GPP] Plane %d sent to buffer\n", (notif_payload & PLANE_MASK));
}
...
STATIC Void pool_notify_Notify (Uint32 eventNo, Pvoid arg, Pvoid info)
{
    ...
    {
        uint32_t plane = (uint32_t)info & PLANE_MASK;
        printf("[DSP] Plane %u computed and sent back.. %u\n", plane, (uint32_t)info);
    }
	#endif
